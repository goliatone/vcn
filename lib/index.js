...
Vecna.prototype.put = function(id, password, filepath) {
    ...
    var targetpath = this.groupId + '/' + id + '.' + this.extension;
...
Vecna.prototype.get = function(id, password){
    var targetpath = this.groupId + '/' + id + '.' + this.extension;
...
Vecna.exec = function(command, options){
    ...
    switch (command) {
        case 'put':
            vcn.put(options.id, options.password, options.filepath).then(function(c){
...
// test.put('menagerie', 'peperone' , '/Users/goliatone/Development/NODEJS/vecna/package.json').then(function(){
//     console.log('DONE');
// });
//
test.get('menagerie', 'peperone').then(function(c){
    console.log('DONE', JSON.parse(JSON.parse(c).content));
});
*/
