            ...
            return awscreds.knox(self.bucket).then(function(client){
                ...
                return client.putFileAsync(tmpfile.name, targetpath).then(function(res){
                    if(res.statusCode > 300){
                        return Promise.reject(new Error('Failed request: StatusCode ' + res.statusCode + '. ' + res.statusMessage));
                    }
    ...
    return awscreds.knox(self.bucket).then(function(client){
        return client.getFileAsync(targetpath).then(function(res){
            ...
            return new Promise(function(resolve, reject){
                ...
                if(res.statusCode > 300){
                    return reject(new Error('Failed request: StatusCode ' + res.statusCode + '. ' + res.statusMessage));
                }
...
Vecna.exec = function(command, options){
    if(!command.match(/put|get|del|list/)) return Promise.reject(new Error('Command not supported'));
